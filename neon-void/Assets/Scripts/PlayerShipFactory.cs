...
public static class PlayerShipFactory
{
    public static GameObject Build(Vector3 pos)
    {
        ...
        // the real Blender-modeled ship (Resources/egoship.fbx, from the
        // reference art); the old procedural saucer stays as a fallback
        var model = EgoShipModel.Attach(ship, tint);
        if (model == null)
            BuildProceduralHull(ship, tint, hullMat, darkMat, cyan, pink, glassMat);

        // engine glow + trails — exhaust positions differ between the two hulls
        float engX = model == null ? 1.7f : 0.85f;
        float engY = model == null ? -0.28f : 0.02f;
        float engZ = model == null ? -1.75f : -2.05f;
        foreach (float side in new[] { -1f, 1f })
        {
            ...
            glow.transform.localPosition = new Vector3(side * engX, engY, engZ);
            ...
            trailGo.transform.localPosition = new Vector3(side * engX, engY, engZ - 0.05f);
    ...
    // the original hand-built saucer — used only when Resources/egoship.fbx
    // is missing, so the game never ships without a player model
    static void BuildProceduralHull(GameObject ship, ShipTint tint,
        Material hullMat, Material darkMat, Material cyan, Material pink, Material glassMat)
    {
