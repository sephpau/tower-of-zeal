...
public static class DoomHull
{
    ...
    public static bool Mount(GameObject ship)
    {
        var parent = ship.transform.Find("visual");
        if (parent == null) parent = ship.transform;
        if (parent.Find("doomHull") != null) return true;
        var prefab = Resources.Load<GameObject>("ships/boss_doom");
        ...
        model.transform.localRotation = Quaternion.AngleAxis(180f, Vector3.up) * model.transform.localRotation;
        ...
        var tex = Resources.Load<Texture2D>("ships/tex/boss_doom_basecolor");
