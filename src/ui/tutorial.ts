...
function renderPickStep(root: HTMLElement, onConfirm: (unitId: string, classId: string) => void, onSkip: () => void, mode: "first-run" | "replay" = "first-run"): void {
  ...
  const intro = isReplay
    ...
    : "Welcome to Gauntlet Tower! Let's start with a quick walkthrough. Pick one unit and one class. <strong>All progress made during this tutorial will reset when it ends.</strong>";
